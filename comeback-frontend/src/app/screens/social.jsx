...
import { quickPrompts, patterns, milestones, circle } from '../data.js';
      ...
      <div className="screen-pad scroll" style={{ paddingTop: 10 }}>
        ...
        {circle.map(m => (
          <div key={m.id} className="card" style={{ padding: 15, marginBottom: m.self ? 12 : 10, border: m.self ? '2px solid #C8F25C' : '1px solid #DDDDD9' }}>
            ...
            {m.detail && <><div style={{ fontSize: 12, color: '#8A8A85' }}>{m.detail}</div><div style={{ display: 'flex', gap: 16, fontSize: 12, color: '#8A8A85', marginTop: 10 }}><span>👊 {m.fist}</span><span>💬 {m.comment}</span></div></>}
          </div>
        ))}
      </div>
    </div>
